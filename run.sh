#!/bin/bash

# Shinchan × Doraemon Remotion Video Generator
# CLI tool for the video generation pipeline

set -e

PROJECT_ROOT="$(cd "$(dirname "$0")" && pwd)"

echo "--------------------------------------------------"
echo "🎬 Remotion Video Generator"
echo "--------------------------------------------------"

# ── Help / Usage ──────────────────────────────────────────────────────────────
show_help() {
    echo ""
    echo "Usage: ./run.sh [command]"
    echo ""
    echo "Commands:"
    echo "  pipeline     Run full pipeline: script → voicebox audio → render video"
    echo "  studio       Open Remotion Studio (preview) at http://localhost:3000"
    echo "  build        Render the current script.json into an MP4 video"
    echo "  clean        Remove all generated files (audio, videos, script.json)"
    echo "  help         Show this help message"
    echo ""
    echo "Examples:"
    echo "  ./run.sh pipeline        # Generate audio + render video from script_files/"
    echo "  ./run.sh studio          # Preview in Remotion Studio"
    echo "  ./run.sh clean           # Wipe all generated files"
    echo ""
    echo "Pipeline flow:"
    echo "  script_files/*.md  →  Voicebox (TTS)  →  script.json  →  Remotion → output/videos/*.mp4"
    echo ""
    echo "Setup:"
    echo "  1. Ensure Voicebox is running: https://github.com/jamiepine/voicebox"
    echo "  2. Add a .md script to script_files/"
    echo "  3. Run: ./run.sh pipeline"
    echo ""
}

# ── Ensure Python venv ─────────────────────────────────────────────────────────
ensure_venv() {
    local VENV_DIR="$PROJECT_ROOT/.venv"
    if [ ! -d "$VENV_DIR" ]; then
        echo "🐍 Setting up Python virtual environment..."
        python3 -m venv "$VENV_DIR"
        "$VENV_DIR/bin/python" -m pip install requests flask flask-cors mutagen --quiet
        echo "   ✅ Virtual environment ready"
    fi
    echo "$VENV_DIR/bin/python"
}

# ── Commands ──────────────────────────────────────────────────────────────────

run_pipeline() {
    echo "🚀 Running full pipeline..."
    PYTHON=$(ensure_venv)
    "$PYTHON" "$PROJECT_ROOT/scripts/pipeline.py"
}

run_studio() {
    echo "🌐 Opening Remotion Studio..."
    echo "   Visit http://localhost:3000 once it starts"
    cd "$PROJECT_ROOT" && npx remotion studio src/index.ts
}

run_build() {
    echo "🎬 Rendering video..."
    cd "$PROJECT_ROOT" && npx remotion render src/index.ts DynamicVideo --overwrite
    echo "   ✅ Video rendered! Check output/videos/"
}

run_clean() {
    echo "🧹 Cleaning all generated files..."
    rm -f "$PROJECT_ROOT"/public/audio/*.mp3
    rm -f "$PROJECT_ROOT"/src/script.json
    rm -f "$PROJECT_ROOT"/public/script.json
    rm -f "$PROJECT_ROOT"/output/scripts/*.json
    rm -f "$PROJECT_ROOT"/output/videos/*.mp4
    rm -rf "$PROJECT_ROOT"/public/cartoon_characters
    echo "   ✅ Clean complete!"
}

run_help() {
    show_help
}

# ── Main ──────────────────────────────────────────────────────────────────────

case "${1:-}" in
    pipeline)    run_pipeline ;;
    studio)      run_studio ;;
    build)       run_build ;;
    clean)       run_clean ;;
    help|--help|-h) run_help ;;
    "")
        # No args - show menu
        show_help
        echo "Choose an option:"
        echo "  [p] Run Full Pipeline"
        echo "  [s] Open Studio (Preview)"
        echo "  [b] Build Video"
        echo "  [c] Clean Generated Files"
        echo "  [q] Quit"
        read -p "> " choice
        case "$choice" in
            [pP]*) run_pipeline ;;
            [sS]*) run_studio ;;
            [bB]*) run_build ;;
            [cC]*) run_clean ;;
            *) echo "Exiting."; exit 0 ;;
        esac
        ;;
    *)
        echo "Unknown command: $1"
        show_help
        exit 1
        ;;
esac
